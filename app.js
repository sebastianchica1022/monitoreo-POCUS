...
// ---------- Persistencia multi-caso ----------
const CASES_KEY = "pocus_hemo_cases_v1";
const ACTIVE_KEY = "pocus_hemo_active_case_v1";
const LEGACY_KEY = "pocus-hemo-case-v1"; // versión anterior de un solo caso
...
function emptyFields() {
  const f = {};
  FIELD_IDS.forEach((id) => { f[id] = null; });
  f.pul_izq_deslizamiento = true;
  f.pul_der_deslizamiento = true;
  return f;
}

function caseLabel(c) {
  if (c.fields && c.fields.patientTag) return c.fields.patientTag;
  const d = new Date(c.updatedAt || Date.now());
  return "Caso " + d.toLocaleDateString("es-CO") + " " + d.toLocaleTimeString("es-CO", { hour: "2-digit", minute: "2-digit" });
}
...
function migrateLegacyIfNeeded() {
  ...
  if (legacyRaw) {
    try {
      const legacyFields = JSON.parse(legacyRaw);
      const id = makeId();
      cases = [{ id, fields: Object.assign(emptyFields(), legacyFields), updatedAt: Date.now() }];
      saveCases(cases);
      setActiveId(id);
      localStorage.removeItem(LEGACY_KEY);
      return cases;
    } catch (e) { /* datos previos corruptos, se ignoran */ }
  }
  const id = makeId();
  cases = [{ id, fields: emptyFields(), updatedAt: Date.now() }];
  saveCases(cases);
  setActiveId(id);
  return cases;
}
...
function applyFieldsToForm(fields) {
  FIELD_IDS.forEach((id) => { setVal(id, fields ? fields[id] : null); });
  toggleDerrameDetalle();
}
function readFieldsFromForm() {
  const f = {};
  FIELD_IDS.forEach((id) => { f[id] = getVal(id); });
  return f;
}
...
function persistCurrentCase() {
  ...
  if (idx === -1) cases.push({ id, fields, updatedAt: Date.now() });
  else { cases[idx].fields = fields; cases[idx].updatedAt = Date.now(); }
  saveCases(cases);
  renderCaseSelector();
...
function switchToCase(id) {
  const c = loadCases().find((x) => x.id === id);
  if (!c) return;
  setActiveId(id);
  applyFieldsToForm(c.fields);
  render();
  renderCaseSelector();
}
...
function createNewCase() {
  const id = makeId();
  const cases = loadCases();
  cases.push({ id, fields: emptyFields(), updatedAt: Date.now() });
  saveCases(cases);
  setActiveId(id);
  applyFieldsToForm(emptyFields());
  render();
  renderCaseSelector();
}
...
function deleteCurrentCase() {
  ...
  let cases = loadCases();
  if (cases.length <= 1) {
    if (!confirm("Este es el único caso guardado. ¿Vaciar sus datos?")) return;
    cases = [{ id, fields: emptyFields(), updatedAt: Date.now() }];
    saveCases(cases);
    applyFieldsToForm(cases[0].fields);
    render();
    renderCaseSelector();
    return;
  }
  if (!confirm("¿Eliminar este caso? Esta acción no se puede deshacer.")) return;
  cases = cases.filter((c) => c.id !== id).sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
  saveCases(cases);
  const next = cases[0];
  setActiveId(next.id);
  applyFieldsToForm(next.fields);
  render();
  renderCaseSelector();
}

function renderCaseSelector() {
  const sel = $("caseSelector");
...
$("caseSelector").addEventListener("change", (e) => switchToCase(e.target.value));
...
let __activeId = getActiveId();
let __cases = loadCases();
if (!__activeId || !__cases.find((c) => c.id === __activeId)) {
  __activeId = __cases[0] ? __cases[0].id : null;
  if (!__activeId) {
    __activeId = makeId();
    __cases.push({ id: __activeId, fields: emptyFields(), updatedAt: Date.now() });
    saveCases(__cases);
  }
  setActiveId(__activeId);
}
const __activeCase = loadCases().find((c) => c.id === __activeId);
applyFieldsToForm(__activeCase.fields);
renderCaseSelector();
render();
